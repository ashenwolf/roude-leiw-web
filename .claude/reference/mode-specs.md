...
- **Schedules fill-blank Slots only when the current lesson declares `@fill`.** `lessonSlotTypeDistribution` takes a `hasFills` flag: false returns the two-bucket table unchanged (so every fill-free lesson — all of A1 — plans identically, rng consumption order included), true appends a `fill-blank` bucket whose share is carved out of **sentence-builder's**, never word-match's (`LESSON.fillShare` of the non-word-match remainder). FillBlank Slot: 1 `FillEntry` via `LESSON.buckets.fillLesson` (same `not-yet-mastered` / `current` / `previous` shape as the other two), direction from `LESSON.buckets.direction`. Fill pools are flat Element lists rather than lesson pools, because a previous lesson may legitimately have no fills at all.
  > Before this existed, a course `.letz` carrying `@fill` was a **trap**: the planner never scheduled one, so that Element could never reach the gate and — since the threshold is 1.0 — its lesson was permanently unpassable. `tests/src/exercise/modes/lesson.test.ts` > "fill reachability" is the guard; it was verified by planting the old behaviour and confirming it fails.

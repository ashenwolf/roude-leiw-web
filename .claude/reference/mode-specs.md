...
- SentenceBuilder / FillBlank Slot: one **phrase** (a `@sentence` or a `@fill`) via `LESSON.buckets.phraseLesson`; `pickPhrase` picks a lesson from the bucket, then one phrase uniformly within it; direction from `LESSON.buckets.direction`.
...
- **A `@sentence` and a `@fill` are both *phrases*** — one Element, one Slot, one graded decision, `+1 correct` per appearance, both directions summed by the gate. So there are **two** slot types, not three: `word-match` and `phrase`. One bucket table (`LESSON.buckets.phraseLesson`), one repeat budget, one `pickPhrase` primitive; the only per-kind dispatch is which Exercise a phrase becomes (`buildPhraseExercise`). A lesson's fills therefore get exposure in proportion to how many it declares — no share constant, and a fill-free lesson cannot yield one because it has none to draw. `lessonSlotTypeDistribution`'s phrase count includes fills, so a fill-heavy lesson does not over-weight word-match.
  > This is why a course `@fill` is safe now. It previously was not: the planner had no fill branch, so such an Element could never reach the gate and — the threshold being 1.0 — its lesson was permanently unpassable. Guard: `tests/src/exercise/modes/lesson.test.ts` > "fill reachability", including a fill-only lesson planning identically to a sentence-only one. All 21 A1 lessons were verified to plan byte-identically across this change.
...
- Direction: rolled with the Lesson direction table, then normalized by `resolveQuestionDirection` — an element carrying `question` is **always** en→lu. That rule is Layer 1, not Mode-specific, so course lessons using `@question` behave identically. It applies to **fills as well as sentences** (Sep 2026): a Q&A fill is en→lu whatever was rolled, and therefore has one presented direction and one stat key instead of two. A question-free fill rolls freely from the same table.

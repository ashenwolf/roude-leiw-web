...
- [Picture-description theme](picture-description-theme.md) — `picture` exam theme: one sub-lesson per photo (not the vocab→phrases→Q&A path), deliberately A1–A2 against the theme-wide ~B1 guideline, LOD-verified vocab; why distractors must be checked through `buildSentenceExercise` rather than by eye; no image is displayed in-app yet (Aug 2026)

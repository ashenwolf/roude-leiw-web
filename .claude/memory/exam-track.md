...
- **The `picture` theme is deliberately off-pattern and A1–A2**, breaking two
  otherwise-uniform conventions. See [[picture-description-theme]] for why: the
  three-step vocab→phrases→Q&A split doesn't fit picture description (each
  sub-lesson is self-contained vocab + descriptions + `@question` prompts,
  split by exam sub-task — whole scene, then one person's appearance), and the
